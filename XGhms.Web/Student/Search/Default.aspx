<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="Default.aspx.cs" Inherits="XGhms.Web.Student.Search.Default" %>
        ...
        <div class="span9">
            <div class="row-fluid">
                <div class="page-header">
				    <h1> <span id="title_h1">查询用户 </span><small> <span id="title_h1small"> 精确查询</span></small></h1>
			    </div>
                <h3>输入查询:</h3><br />
...
<script type="text/javascript">
    $(function () {
        $("#left_nav_ul li:eq(9)").attr('class', 'active'); //获取相应的li，设置class为active
    });
    ...
    function showUserList() {
        ...
        $.ajax({
            ...
            error:function (){
                ...
                $('#userListTb').append('<tr class="error"><td colspan="4">查询出现错误！</td></tr>');
            }
        });
    }
    ...
    function addUserListTb(id,userID,userName,collegeName) {
        $('#userListTb').empty(); //先清除
        $('#userListTb').append('<tr class="pending-user"><td>' + userID + '</td><td>' + userName + '</td><td>' + collegeName + '</td><td><span class="user-actions"><a href="javascript:showUserInfo(' + id + ');" class="label label-info"><i class="icon-search"></i> 查看详细</a></span></td></tr>');
    }

    function userDontExists() {
        $('#userListTb').empty(); //先清除
        $('#userListTb').append('<tr><td colspan="4">没有找到该用户</td></tr>');
    }
