<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="MessageInfo.aspx.cs" Inherits="XGhms.Web.Admin.Message.MessageInfo" %>
...
<script type="text/javascript">
    $(function () {
        $("#left_nav_ul li:eq(14)").attr('class', 'active'); //获取相应的li，设置class为active
    ...
    function RepeaterMsg() {
        var smsgCon = escape(editor.html());
        var re = new RegExp('\\+', 'g');
        var msgCon = smsgCon.replace(re, '%2b');
        var mesid = getParam("id");
        $.ajax({
            type: "post",
            dataType: "json",
            url: "/Handles/MessageHandler.ashx",
            data: "action=InsertMsgByOtherID&id=" + mesid + "&msgCon=" + msgCon,
            cache: false,
            async: true,
            success: function (data) {
                alert(data.msg);
